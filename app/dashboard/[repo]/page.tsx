...
/**
 * Returns styling based on PR state.
 */
...
export default function RepoAnalyticsPage() {
  ...
  // params.repo is the dynamic segment from the URL
  const repoName = params.repo as string

  // We need the GitHub username to pass as owner
  // It's stored in the session from our auth setup
          ...
          <>
            ...
            <div className="mb-8 grid grid-cols-2 gap-3 md:grid-cols-4">
              {[
                {
                  label: 'Total PRs',
                  value: data.metrics.totalPRs,
                  icon: GitPullRequest,
                  color: 'var(--accent)',
                },
                {
                  label: 'Merge Rate',
                  value: `${data.metrics.mergeRate}%`,
                  icon: GitMerge,
                  color: 'var(--green)',
                },
                {
                  label: 'Avg Cycle Time',
                  value: formatHours(data.metrics.avgCycleTimeHours),
                  icon: Clock,
                  color: '#f97316',
                },
                {
                  label: 'Avg First Review',
                  value: formatHours(data.metrics.avgTimeToFirstReviewHours),
                  icon: CheckCircle,
                  color: '#60a5fa',
                },
              ].map((card) => (
            ...
            {data.metrics.contributorStats.length > 0 && (
              ...
              >
                ...
                <div className="divide-y" style={{ borderColor: 'var(--border)' }}>
                  {/* Table header */}
                  <div
                    className="mono grid grid-cols-3 px-5 py-2 text-xs"
                    style={{ color: 'var(--text-3)' }}
                  >
                    <span>Username</span>
